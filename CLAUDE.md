...
│   ├── tithiMessages.js       # 30 tithi daily reflection messages keyed by continuous tithi number (1–30); VARA_NAMES array; getVara(date) helper
│   ├── shareCard.js           # Canvas share-card generator — generateShareCard({ phase, vara, dateLabel, tithiName, paksha, illuminationPct, message }) → Promise<Blob>; 540×960px PNG (9:16); shadow-overlay moon mirrors MoonVisual.jsx; gold-on-dark QR code via qrcode npm package
│   ├── assets/
│   │   └── moon-texture.png   # Full-moon PNG texture used by MoonVisual.jsx; bundled by Vite; clipped to disc shape inside SVG
│   ├── pages/
...
│       ├── TodayMessageModal.jsx # Bottom-sheet "View today's message" — shows vara, date, tithi, paksha, illumination, daily message; single Share action generates PNG share card via shareCard.js and calls navigator.share({ files })
...
- `Home.jsx` also samples moon **phase and illumination** at local sunrise (via `getSunriseForDate`) so the moon visual stays consistent with the tithi label on boundary days. **Do NOT revert to midnight sampling** — it was the original bug.
